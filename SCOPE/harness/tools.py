...
class ToolSchema(BaseModel):
    ...
    def _to_openai_format(self) -> Dict[str, Any]:
        """Convert to OpenAI Chat Completions function-calling format.

        Nested ``function`` payload is required by OpenAI chat.completions and
        vLLM; the flat Responses-style shape is rejected by vLLM with 400.
        """
        return {
            "type": "function",
            "function": {
    ...
    # Alias kept for ProviderFormat.QWEN_MOONSHOT / OPENAI_HARMONY callers.
    def _to_qwen_moonshot_format(self) -> Dict[str, Any]:
        """Same nested shape as OpenAI chat.completions tools."""
