...
class OpenAIAgentInferenceModel(AgentInferenceModel):
    ...
    def _call_chat_completions(self, context: InferenceContext) -> Optional[Action]:
        trajectory = context.trajectory
        toolset = context.toolset
        # Cap by configured per-turn output limit. context.max_tokens is the
        # remaining episode budget and can exceed vLLM max_model_len.
        max_tokens = self.max_output_tokens
        if context.max_tokens is not None:
            max_tokens = max(1, min(int(context.max_tokens), int(self.max_output_tokens)))
        ...
        if choice.finish_reason == "stop":
            ...
            # Soft-parse: unknown tools / bad JSON are salvaged via action_repair
            # when Ultra tools (curate/end_search) are present; otherwise raise.
            ...
            curate_tool = toolset.get_tool("curate")
            end_search_tool = toolset.get_tool("end_search")
            if curate_tool is not None and end_search_tool is not None:
                repaired = repair_action_from_tool_calls(
                    tool_calls=raw_calls,
                    toolset=toolset,
                    curate_tool=curate_tool,
                    end_search_tool=end_search_tool,
                )
                if repaired.tools:
                    return repaired
                # Fall through to strict parse for clearer errors if repair empty
