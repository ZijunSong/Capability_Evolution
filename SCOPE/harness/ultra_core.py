...
RECALL_BETA = 2.0
# Optional citation validity term (SCOPE). Default 0 keeps legacy reward unchanged.
ALPHA_CITATION = float(os.environ.get("ALPHA_CITATION", "0.0"))
...
class WorkingMemory:
    ...
    def __init__(self, query: str, normalize_ids: bool = True):
        ...
        # Verification records (M2 module)
        self.verification_records: List[Any] = []

        # SCOPE observation lineage (read-only export; does not change tool semantics)
        self.observation_lineage: List[Dict[str, Any]] = []
        self._obs_counter: int = 0
        # curated_doc_id -> observation_ids that justified curation
        self.curated_observation_ids: Dict[str, List[str]] = {}
        # claim_id -> claim metadata for DecisionState export
        self.claim_states: Dict[str, Dict[str, Any]] = {}
    ...
    def record_observation(
        ...
        *,
        ...
        """Append an observation lineage record; returns observation_id."""
        self._obs_counter += 1
        obs_id = f"obs_t{self.turn_number}_{self._obs_counter}"
        text_hash = hashlib.sha256((text or "").encode("utf-8")).hexdigest()[:16]
        self.observation_lineage.append(
            {
                "observation_id": obs_id,
                "source_type": source_type,
                "source_document_ids": list(source_document_ids or []),
                "created_turn": int(self.turn_number),
    ...
    def bind_curated_observations(
        self, doc_ids: List[str], observation_ids: List[str]
    ) -> None:
        for doc_id in doc_ids:
            norm = self._normalize_id(str(doc_id))
            existing = self.curated_observation_ids.setdefault(norm, [])
            for oid in observation_ids:
                if oid not in existing:
                    existing.append(oid)

    def upsert_claim_state(
        self,
        claim_id: str,
        text: str,
        status: str,
        ...
        self.claim_states[claim_id] = {
            "claim_id": claim_id,
            "text": text,
            "status": status,
            "supporting_document_ids": list(supporting_document_ids or []),
            "source_observation_ids": list(source_observation_ids or []),
        }

    def snapshot_hash(self) -> str:
        """Stable hash of student-visible WM structure (no full doc_store)."""
        payload = json.dumps(
            {
                "turn": self.turn_number,
                "curated": list(self.curated_ids),
                "pool": list(self.pool_ids),
                "history": list(self.search_history),
                "obs": [r["observation_id"] for r in self.observation_lineage],
                "claims": sorted(self.claim_states.keys()),
            },
            sort_keys=True,
        ).encode("utf-8")
    ...
    def curate(
        ...
        for rid in remove_set_all:
            ...
            self.curated_observation_ids.pop(rid, None)
    ...
    def get_minimal_state(self) -> str:
        """Minimal evidence state: curated selection only (M1 fallback)."""
        lines = [
            f'Query: "{self.query}"',
            f"Curated Set ({len(self.curated_ids)}/{MAX_CURATED_DOCS}):",
        ]
        if self.curated_ids:
    ...
    def get_structured_state(self, *, include_verification: bool = False) -> str:
        """Full structured WM text for student/teacher views."""
        text = self.to_text()
        if include_verification and self.verification_records:
            records = self.verification_records[-5:]
            compact = "\n".join(
                getattr(r, "compact_text", lambda: str(r))()
                if hasattr(r, "compact_text")
                else str(r)
                for r in records
            )
    ...
    def get_privileged_state(self) -> dict[str, Any]:
        """Privileged state for teacher view (includes verification rationales)."""
        records = []
        ...
        return {
            ...
            "evidence_graph": (
                self.evidence_graph.render_summary()
                if self.evidence_graph is not None
                else ""
            ),
        }
...
def compute_reward(
    ...
    alpha_citation: float | None = None,
    ...
    if REWARD_VERSION == "v4":
        ...
        alpha = ALPHA_CITATION if alpha_citation is None else float(alpha_citation)
        if alpha != 0.0:
            citation_term = alpha * float(citation_score)
            reward = reward + citation_term
            metrics["citation_score"] = float(citation_score)
            metrics["citation_term"] = citation_term
    ...
    if alpha != 0.0:
        citation_term = alpha * float(citation_score)
        final_reward = final_reward + citation_term
        metrics["citation_score"] = float(citation_score)
        metrics["citation_term"] = citation_term
