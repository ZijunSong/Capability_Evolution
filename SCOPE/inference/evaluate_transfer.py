...
while swapping retrieval backends by dataset:
  - longsealqa: Chroma tools (same as in-domain flow)
  - seal0qa: Serper search + Serper/Jina read fallback
  - frames, hotpotqa_subset: Wikipedia Serper search + wiki/Serper/Jina read fallback
"""
...
try:
    from eval_scripts.web_tools import WebToolSet, require_serper_health
    from eval_scripts.wiki_tools import WikiToolSet
except ImportError:
    WebToolSet = None  # type: ignore[assignment]
    WikiToolSet = None  # type: ignore[assignment]
    require_serper_health = None  # type: ignore[assignment]

logger = structlog.get_logger("evaluate_transfer")
...
WEB_TRANSFER_DATASETS = {"seal0qa"}
WIKI_TRANSFER_DATASETS = {"frames", "hotpotqa_subset"}
TRANSFER_DATASETS = {"longsealqa"} | WEB_TRANSFER_DATASETS | WIKI_TRANSFER_DATASETS
...
async def _eval_single_query(
    ...
    if disable_id_normalization:
        # URL IDs can legitimately contain underscores; avoid doc-id suffix stripping.
        env._normalize_ids = False
        env.wm.normalize_ids = False
...
async def eval_checkpoint(
    ...
    max_turns: int,
    parallel: int = 10,
    disable_id_normalization: bool = False,
) -> List[Dict]:
...
def print_results_table(name: str, results: List[Dict]) -> None:
    ...
    print(
        f"  {'QID':<12} {'Recall':>7} {'PoolRec':>8} {'Prec':>6} {'Reward':>7} {'Cur':>4} {'Pool':>5} {'T':>3} {'Err':>4}"
    )
...
def _build_tooling(
    ...
    dataset: SearchDataset,
    text_token_counter,
) -> Tuple[ToolSet, Any, bool]:
    """Build toolset + search tool with dataset-specific retrieval backend."""
    config = get_config()
    ...
    if dataset_name in WEB_TRANSFER_DATASETS:
        if WebToolSet is None or require_serper_health is None:
            raise RuntimeError("Web transfer tools are unavailable in this environment.")
        require_serper_health(timeout_seconds=args.serper_health_timeout)
        ...
        if search_tool is None:
            raise RuntimeError("search_corpus tool is missing from WebToolSet")
        return toolset, search_tool, True

    if dataset_name in WIKI_TRANSFER_DATASETS:
        if WikiToolSet is None or require_serper_health is None:
            raise RuntimeError("Wiki transfer tools are unavailable in this environment.")
        require_serper_health(timeout_seconds=args.serper_health_timeout)
        ...
        search_tool = toolset.get_tool("search_corpus")
        if search_tool is None:
            raise RuntimeError("search_corpus tool is missing from WikiToolSet")
        return toolset, search_tool, True
    ...
    return toolset, search_tool, False
...
async def main() -> None:
    ...
    parser.add_argument(
        "--dataset",
        default="longsealqa",
        choices=sorted(TRANSFER_DATASETS),
        help="Transfer dataset name",
    )
    parser.add_argument(
        "--split",
        default="test",
    ...
    parser.add_argument(
        ...
        help="Which retrieval corpus split the tools should use (Chroma datasets only)",
    )
    ...
    parser.add_argument(
        "--serper-health-timeout",
        type=int,
        default=20,
        help="Serper preflight timeout in seconds for web/wiki datasets",
    )
    parser.add_argument(
        "--reranker",
        type=str,
    ...
    toolset, search_tool, disable_id_normalization = _build_tooling(
        args=args,
        dataset=dataset,
        text_token_counter=text_token_counter,
    )
    ...
    for ckpt_name, ckpt_path in checkpoints.items():
        ...
        results = await eval_checkpoint(
            ...
            disable_id_normalization=disable_id_normalization,
        )
