...
if [[ ! -f "${DECISION}" ]]; then
  r11_log "ERROR: missing Phase A decision ${DECISION}"
  exit 2
fi
VIEW=$(python -c "import json; print(json.load(open('${DECISION}'))['selected_stage1_view'])")
...
# Build factorized eval for this variant's stage1 view (idempotent)
r11_log "${VARIANT}: ensure factorized eval view=${VIEW} (and variant-specific)"
# Determine variant view from train config after train; prebuild selected + controls.
for vbuild in "${VIEW}" A0 A1 A2 A3; do
  if [[ ! -f "${DATA_DIR}/factorized_eval/${vbuild}/base_live.jsonl" ]]; then
    python training/scope_round11/build_factorized_eval.py --view "${vbuild}" \
      >> "${LOG_DIR}/build_factorized_eval_${vbuild}.log" 2>&1 || true
  fi
done

r11_log "${VARIANT} train on GPU${GPU}"
start_hb
...
# Resolve actual stage1 view used by this variant
VVIEW=$(python -c "import json; print(json.load(open('${VDIR}/train_only_report.json')).get('stage1_view','${VIEW}'))")
...
# checkpoint-only variants still rank with stage2; operation head may be weak — still score both.
...
run_eval_split() {
  ...
  rm -f "${outp}"
  r11_log "${VARIANT} factorized-vLLM ${tag} view=${VVIEW}"
  start_hb
