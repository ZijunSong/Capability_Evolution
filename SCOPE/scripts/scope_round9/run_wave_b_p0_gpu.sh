#!/usr/bin/env bash
# P0 Wave B: train+eval one main hier seed into outputs/scope_round9/wave_b_p0/
set -euo pipefail
...
WAVE_B_OUT="${OUT}/wave_b_p0"
VDIR="${WAVE_B_OUT}/${VARIANT}"
MARKER="${VDIR}/DONE"
VALID_FROZEN="${DATA_DIR}/frozen_replay/offline_valid.jsonl"
HOLDOUT_REPLAY_IN="${DATA_DIR}/frozen_replay/base_live.jsonl"

if [[ -f "${MARKER}" ]]; then
  scope9_log "Skip Wave B P0 ${VARIANT}"
  exit 0
fi

mkdir -p "${VDIR}/eval_offline_valid" "${VDIR}/eval_holdout"

scope9_log "P0 ${VARIANT} train (disable_replan + resampled CONTINUE)"
CUDA_VISIBLE_DEVICES="${GPU}" python training/scope_round9/run_wave_b_train.py \
  --variant "${VARIANT}" --gpu cuda:0 \
  --out-root "${WAVE_B_OUT}" --disable-replan --force-retrain \
  >> "${LOG_DIR}/wave_b_p0_${VARIANT}.log" 2>&1

MODEL="${VDIR}/merged"
if [[ ! -f "${MODEL}/config.json" ]]; then
  scope9_log "ERROR: missing merged model for P0 ${VARIANT}: ${MODEL}"
  exit 1
fi
...
run_eval_split() {
  ...
  if [[ ! -f "${hf_out}" ]] || [[ "$(wc -l < "${hf_out}" | tr -d ' ')" -lt "${n_expected}" ]]; then
    rm -f "${hf_out}"
    scope9_log "P0 ${VARIANT} HF eval ${split}"
    CUDA_VISIBLE_DEVICES="${GPU}" python training/scope_round9/replay_frozen_hf.py \
      ...
      >> "${LOG_DIR}/wave_b_p0_${VARIANT}_${split}_hf.log" 2>&1
  fi
  if [[ ! -f "${vllm_out}" ]] || [[ "$(wc -l < "${vllm_out}" | tr -d ' ')" -lt "${n_expected}" ]]; then
    rm -f "${vllm_out}"
    scope9_log "P0 ${VARIANT} vLLM eval ${split}"
    CUDA_VISIBLE_DEVICES="${GPU}" python training/scope_round9/run_vllm_replay_split.py \
      ...
      >> "${LOG_DIR}/wave_b_p0_${VARIANT}_${split}_vllm.log" 2>&1
  fi
}
...
touch "${MARKER}"
scope9_log "Wave B P0 ${VARIANT} DONE"
