...
class SlidingWindowSearchEnv(Env):
    ...
    def __init__(
        ...
        # SCOPE action history for DecisionState export
        self._action_records: List[ActionRecord] = []
        self._episode_id: str = f"{query_id}_r{rollout_idx}"

    # ── SCOPE DecisionState export ─────────────────────────────────────────

    def export_visible_state(self) -> dict:
        """Student-visible state only (no full doc_store / future observations)."""
        return {
            "episode_id": self._episode_id,
            "task_id": self.query_id,
            "turn_id": self._current_turn,
            "query": self.query_text,
            "pool_document_ids": list(self.wm.pool_ids),
            "curated_document_ids": list(self.wm.curated_ids),
            "observation_ids": [
                r["observation_id"] for r in self.wm.observation_lineage
                if r.get("visible_to_student", True)
            ],
            "visible_document_ids": list(self.wm.pool_ids),
            "wm_snapshot_hash": self.wm.snapshot_hash(),
        }

    def export_decision_state(self) -> DecisionState:
        """Immutable DecisionState snapshot for shadow modules (no env reference)."""
        obs_records = [
            ObservationRecord(
                observation_id=r["observation_id"],
                source_type=r.get("source_type", "other"),
                source_document_ids=tuple(r.get("source_document_ids", [])),
                created_turn=int(r.get("created_turn", 0)),
                visible_to_student=bool(r.get("visible_to_student", True)),
                text_hash=str(r.get("text_hash", "")),
            )
            for r in self.wm.observation_lineage
        ]
        claims = tuple(
            ClaimState(
                claim_id=c["claim_id"],
                ...
                supporting_document_ids=tuple(c.get("supporting_document_ids", [])),
                source_observation_ids=tuple(c.get("source_observation_ids", [])),
            )
            for c in self.wm.claim_states.values()
        )
        verify_states = []
        for rec in self.wm.verification_records:
            ...
            verify_states.append(
                VerificationRecordState(
                    ...
                    document_ids=tuple(d.get("doc_ids", d.get("document_ids", []))),
                    judgments={
                        str(k): bool(v) for k, v in dict(d.get("judgments", {})).items()
                    },
                    source_observation_ids=tuple(d.get("source_observation_ids", [])),
                )
            )

        last_action = self._action_records[-1].action_type if self._action_records else None
        repeated = self._compute_repeated_query_score()
        rendered = ""
        try:
            rendered = self.wm.to_text()
        except Exception:
            rendered = ""

        last_args = (
            dict(self._action_records[-1].arguments) if self._action_records else {}
        )
        last_q = ""
        if last_action in {"search", "rewrite_query", "continue_search"}:
            last_q = str(last_args.get("query") or "")
            if not last_q and isinstance(last_args.get("queries"), list) and last_args["queries"]:
                last_q = str(last_args["queries"][0])
        ...
        return DecisionState(
            ...
            action_history=tuple(self._action_records),
            observation_ids=tuple(o.observation_id for o in obs_records if o.visible_to_student),
            visible_document_ids=tuple(self.wm.pool_ids),
            pool_document_ids=tuple(self.wm.pool_ids),
            curated_document_ids=tuple(self.wm.curated_ids),
            evidence_claims=claims,
            verification_records=tuple(verify_states),
            remaining_turns=max(0, self.max_turns - self._current_turn),
            remaining_search_calls=None,
            token_budget_used=int(self._approx_prompt_tokens),
            token_budget_total=int(MODEL_CTX_LIMIT),
            last_action_type=last_action,
            repeated_query_score=repeated,
            wm_snapshot_hash=self.wm.snapshot_hash(),
            goal=self.query_text,
            last_action_arguments=last_args,
            last_query=last_q,
            repeated_query_count=1 if (repeated or 0.0) > 0.5 else 0,
    ...
    def _record_student_action(self, action: Action) -> None:
        names: List[str] = []
        ...
        action_type = cap.action_type.value if cap else (names[0] if names else "unknown")
        args = cap.arguments if cap else (params_list[0] if params_list else {})
    ...
    async def step(self, action_tokens: TinkerAction) -> StepResult:
        """Harmony-token step (legacy). Prefer step_from_action for chat/API policies."""
        ...
        return await self.step_from_action(action)

    async def step_from_action(self, action: Action) -> StepResult:
        """Protocol-agnostic step: consume a parsed Action (chat / Harmony / …).

        SCOPE DecisionState export works against this path; message encoding is
        the caller's responsibility (Qwen chat template, Harmony, OpenAI, …).
        """
        ...
        for tool, params, source in zip(action.tools, action.params, action.sources):
            ...
            try:
                ...
                if name == "fan_out_search":
                    ...
                    source_type = "search"
                    if meta is not None and hasattr(meta, "returned_chunk_ids"):
                        source_docs = list(meta.returned_chunk_ids)
                    ...
                    if isinstance(add_ids, list):
                        source_docs = [str(x) for x in add_ids]
                    ...
                    if isinstance(doc_ids, list):
                        source_docs = [str(x) for x in doc_ids]
                    ...
                    output = "Search concluded."
                    obs_builder.add_observation(output, source=source, tool_metadata=None)
                    source_type = "other"
                elif name == "prune_chunks":
                    output = "Context is managed via working memory. No pruning needed."
                    ...
                    obs_builder.add_observation(
                        output, source=source, tool_metadata=None,
                    )

                # SCOPE lineage: record observation id without exposing doc_store
                obs_id = self.wm.record_observation(
                    source_type=source_type,
                    source_document_ids=source_docs,
                    text=output if isinstance(output, str) else str(output),
                    visible_to_student=True,
                )
                if name == "curate" and source_docs:
                    self.wm.bind_curated_observations(source_docs, [obs_id])
                if name == "verify":
                    claim = str(params.get("claim", "")).strip()
                    if claim:
                        claim_id = f"claim_{compute_text_hash(claim)}"
                        self.wm.upsert_claim_state(
                            claim_id,
                            claim,
                            status="verified_pending",
                            supporting_document_ids=source_docs,
                            source_observation_ids=[obs_id],
                        )
                        # Attach observation lineage onto latest verification record if present
                        if self.wm.verification_records:
                            last = self.wm.verification_records[-1]
                            if hasattr(last, "source_observation_ids"):
                                last.source_observation_ids = [obs_id]  # type: ignore[attr-defined]
            except Exception as e:
                logger.warning("tool_error", tool=name, error=str(e)[:200], qid=self.query_id)
                err = f"Error executing {name}: {str(e)[:200]}"
                obs_builder.add_observation(err, source=source, tool_metadata=None)
                self.wm.record_observation(
                    source_type="other",
                    source_document_ids=[],
                    text=err,
                    visible_to_student=True,
    ...
    def _exec_verify(self, params: Dict) -> str:
        ...
        raw = exec_verify_claim(self._openai_client, doc_texts, claim)
        # Persist structured verification record so DecisionState / shadows can
        # distinguish premature vs evidence-sufficient stops.
        try:
            from harness.modules.verification import VerificationRecord

            norm_ids = [self.wm._normalize_id(d) for d in doc_ids]
            rec = VerificationRecord.from_verify_output(
                turn_id=int(self._current_turn),
                claim=claim,
                doc_ids=norm_ids,
                raw_output=raw if isinstance(raw, str) else str(raw),
            )
            self.wm.verification_records.append(rec)
        except Exception as exc:  # noqa: BLE001
            logger.warning(
                "verify_record_store_failed",
                error=str(exc)[:200],
                qid=self.query_id,
            )
        return raw
