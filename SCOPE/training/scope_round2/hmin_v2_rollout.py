#!/usr/bin/env python3
"""H_min_v2 closed-loop rollout for Round 2 (1 GPU, TP=1)."""
...
def _load_shard_queries(manifest_path: Path, shard: str) -> list[str]:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--vllm-port", type=int, default=8800)
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    os.environ["V8D_SUBTRACTIVE_CURATION"] = "0"
    os.environ["V8D_IMPORTANCE_TAGGING"] = "0"
    os.environ["V8D_AUTO_POPULATE_FIRST_SEARCH"] = "0"
    os.environ["V8D_EVIDENCE_GRAPH"] = "0"
    os.environ["V8D_SENTENCE_COMPRESS"] = "0"
    os.environ["V8D_CONTENT_DEDUP"] = "0"
    os.environ["V8D_VERIFY_TOOL"] = "1"
    os.environ["V8D_TOKEN_BUDGET_MARKER"] = "0"
    os.environ["V8D_CHUNK_NEIGHBORS"] = "0"

    harness_cfg = load_harness_config(args.harness_config)
    apply_harness_config(harness_cfg)

    shard_qids = _load_shard_queries(Path(args.manifest), args.shard)
    ...
    summary = {
        ...
        "n_completed": len(records) - len(pending) + len(pending),
        "shard": args.shard,
