...
This directory is a BiSHOP / Harness-1 fork focused on **selective capability
internalization** (fixed Search Harness → same-state shadow guidance → dual-mode
OPD). Primary training entry:

```bash
uv run python training/train_scope.py --mode dry-run --config configs/scope/dual_mode.yaml
```

Configs live under `configs/scope/`. Typed shadow modules live under
`harness/shadow/` and `harness/capability/`. Dual-mode OPD lives under
`training/opd_v2/`. Evaluation helpers live under `inference/scope/`.

**Legacy / future work (frozen for SCOPE paper-1):** `harness/lifecycle/`,
`training/train_coevolution_round.py`, ACTIVE/CONDITIONAL/RETIRED module
retirement, and M4 Recovery. Keep the code; do not import it from
`train_scope.py`. The previous full-trace OPD path (`training/train_opd.py`)
...
- `harness/`: shared search harness, tools, trajectory, task, reranking, and
  configuration modules (`capability/`, `shadow/`, `artifacts/` for SCOPE).
- `configs/scope/`: SCOPE dual-mode OPD and ablation YAML configs.
- `inference/`: Harness-1 evaluation, component ablations, HF inference, and
  vLLM inference utilities.
- `inference/scope/`: SCOPE runtime / transfer / error-slice evaluation.
- `inference/baselines/`: in-domain and transfer baseline evaluation runners.
- `training/`: SFT, RL, legacy OPD, and **SCOPE** (`train_scope.py`, `opd_v2/`).
...
- `tests/` / `tests/scope/`: lightweight smoke tests and SCOPE unit tests.
