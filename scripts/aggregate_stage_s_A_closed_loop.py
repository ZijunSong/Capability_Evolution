...
OUT = SCAPE / "outputs/stage_s/A_auto_fourgrid"
...
def aggregate() -> dict:
    ...
    s1 = load_quality(LOO / "minus_auto_populate_first_search")
    s2 = load_quality(OUT / "S2_trained_minus_auto")
    ...
    grid = {
        ...
        "S1": {"quality": mean(s1, shared), "cost": cost_proxy(LOO / "minus_auto_populate_first_search", 7.0), "label": "theta0+H_-verify", "source": "loo"},
        "S2": {"quality": mean(s2, shared), "cost": cost_proxy(OUT / "S2_trained_minus_auto", 7.0), "label": "theta'+H_-verify", "source": "closed_loop_L64_seed42_hf"},
        "S3": {"quality": mean(s3, shared), "cost": cost_proxy(OUT / "S3_trained_full", 10.0), "label": "theta'+H_full", "source": "closed_loop_L64_seed42_hf"},
        "n_shared": len(shared),
        "verdict": "CLOSED_LOOP",
        "h100_required": False,
        "student_ckpt": str(SCAPE / "outputs/stage_l/A_auto_opd_provisional/L64_seed42_hf/hf_model"),
    ...
    (OUT / "GATE_S_A.json").write_text(json.dumps(gs, indent=2) + "\n")
    (OUT / "CLOSED_LOOP_DONE").write_text("ok\n")
    status = (
        f"# CLOSED_LOOP_STATUS — A auto_populate four-grid (S2/S3)\n\n"
        ...
        f"- student: A_auto_opd_provisional/L64_seed42_hf/hf_model\n"
    )
...
def main() -> None:
    ...
    while time.time() < deadline:
        if ready("S2_trained_minus_auto") and ready("S3_trained_full"):
            aggregate()
            return
        s2n = len((OUT / "S2_trained_minus_auto/harness_rollouts.jsonl").read_text().splitlines()) if (OUT / "S2_trained_minus_auto/harness_rollouts.jsonl").exists() else 0
        ...
        print(f"[wait] S2={s2n}/64 DONE={ready('S2_trained_minus_auto')} S3={s3n}/64 DONE={ready('S3_trained_full')}", flush=True)
        time.sleep(120)
