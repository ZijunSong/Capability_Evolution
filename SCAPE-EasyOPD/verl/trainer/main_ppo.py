...
# [EasyOPD] Compatibility patch: the frozen /opt runtime currently has
# opentelemetry-exporter-prometheus 0.65b0 with semantic-conventions 0.47b0.
# Ray imports the prometheus exporter while starting dashboard modules, and the
# exporter expects this enum to exist. Inject only the missing value before Ray
# imports dashboard modules; this keeps the /opt package set unchanged.
try:
...
class TaskRunner:
    ...
    def add_ref_policy_worker(self, config, ref_policy_cls):
        """Add reference policy worker if any KL-based objective needs teacher log-probs."""
        from verl.trainer.ppo.ray_trainer import Role
