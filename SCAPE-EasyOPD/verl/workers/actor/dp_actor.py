...
if is_cuda_available:
    ...
    # SCAPE smoke environments may validate the non-remove-padding training path
    # before flash-attn is available. Keep upstream behavior when flash-attn is
    # installed, but avoid import-time failure for use_remove_padding=False.
    try:
        from flash_attn.bert_padding import index_first_axis, pad_input, rearrange, unpad_input
    except ModuleNotFoundError:
        from einops import rearrange

        def _flash_attn_required(*_args, **_kwargs):
            raise ModuleNotFoundError("flash_attn is required when use_remove_padding=True")

        index_first_axis = _flash_attn_required
        pad_input = _flash_attn_required
        unpad_input = _flash_attn_required
