...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __enter__(self):
        ...
        with simple_timer("reshard", self.timing):
            ...
            peft_config = None
            peft_model = getattr(self.module, "_fsdp_wrapped_module", self.module)
            hf_model_type = str(getattr(getattr(self.model_config, "hf_config", None), "model_type", ""))
            hf_architectures = [str(value) for value in (getattr(getattr(self.model_config, "hf_config", None), "architectures", None) or [])]
            model_path_hint = str(getattr(self.model_config, "path", "") or getattr(self.model_config, "local_path", ""))
            force_disable_rollout_lora = os.environ.get("SCAPE_DISABLE_VLLM_ROLLOUT_LORA", "0") == "1"
            rollout_lora_supported = not force_disable_rollout_lora and hf_model_type != "qwen3_moe" and "qwen3" not in model_path_hint.lower() and not any("Qwen3Moe" in value for value in hf_architectures)
            if hasattr(peft_model, "peft_config") and rollout_lora_supported:
                peft_config = peft_model.peft_config.get("default", None)
                params = __collect_lora_params()
            elif hasattr(peft_model, "peft_config") and not rollout_lora_supported:
                logger.warning("Skipping FSDP-to-vLLM LoRA synchronization for unsupported model architecture: %s", hf_model_type or hf_architectures)
                params = OrderedDict()
            else:
