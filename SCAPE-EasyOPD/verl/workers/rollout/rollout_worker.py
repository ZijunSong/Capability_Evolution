...
class RolloutWorker(Worker):
    def __init__(self, config: RolloutConfig, model_config: HFModelConfig) -> None:
        ...
        if self.config.name == "vllm":
            ...
            hf_model_type = str(getattr(self.model_config.hf_config, "model_type", ""))
            hf_architectures = [str(value) for value in (getattr(self.model_config.hf_config, "architectures", None) or [])]
            vllm_lora_supported = hf_model_type != "qwen3_moe" and not any("Qwen3Moe" in value for value in hf_architectures)
            lora_kwargs = (
                {"lora_kwargs": {"enable_lora": True, "max_loras": 1, "max_lora_rank": self.model_config.lora_rank}}
                if self.model_config.lora_rank > 0 and vllm_lora_supported
                else {}
            )
            if self.model_config.lora_rank > 0 and not vllm_lora_supported:
                logger.warning("Disabling vLLM rollout LoRA because this model architecture does not support vLLM LoRA: %s", hf_model_type or hf_architectures)
