#!/usr/bin/env python3
"""Evaluate auto_populate_first_search OPD conditions on the strict 384 pool."""
...
def make_prompt(row: dict[str, Any], teacher: bool) -> str:
    state = {
        ...
        "auto_populate_first_search": "enabled" if teacher else "disabled",
    }
    schemas = {
    ...
    if teacher:
        state["runtime_semantics"] = "After the first successful search, the runtime automatically adds its top 8 ranked documents to the curated set."
    return json.dumps({
...
def evaluate(name: str, rows: list[dict[str, Any]], adapter: Path | None, teacher: bool, searcher: Any, out: Path) -> dict[str, Any]:
    ...
    for i, row in enumerate(rows, 1):
        ...
        records.append({
            ...
            "retrieved_docids": retrieved, "auto_populated_docids": activated,
...
def main() -> int:
    ...
    payload = {"status": "AUTO_POPULATE_FIRST_SEARCH_OPD_384_READY", "component": "auto_populate_first_search", "query_count": len(rows), "test_query_count": sum(r["official_split"] == "test" for r in rows), "settings": summaries, "base_model": BASE_MODEL, "retrieval_backend": retrieval_backend, "retrieval": "ordered top-100 docids; official Lucene when available, audited local-corpus fallback otherwise", "auto_populated_top_k": 10, "student_inference_privilege": False}
