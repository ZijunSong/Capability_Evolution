...
def paired_bootstrap_delta(before: list[float], after: list[float], *, seed: int = 20260819, n_boot: int = 1000) -> dict[str, float]:
...
def evaluate_rows(backend, rows: list[dict[str, Any]]) -> list[dict[str, float]]:
    values = []
    for row in rows:
        values.append(backend.score_divergence(
...
def aggregate(values: list[dict[str, float]]) -> dict[str, float]:
    keys = values[0].keys()
    return {k: sum(v[k] for v in values) / max(1, len(values)) for k in keys}
...
def mechanism_audit(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        overlaps.append(float(payload.get("topK_overlap") or 0.0))
        off = payload.get("retrieved_doc_ids_off") or []
        on = payload.get("retrieved_doc_ids_on") or []
        if list(off) != list(on):
            changed += 1
    n = len(rows)
    return {
        "n_rows": n,
        "mean_topK_overlap": sum(overlaps) / max(1, n),
        "retrieval_changed_rate": changed / max(1, n),
        "qrel_recall_delta": "N/A",
...
def main() -> int:
    ...
    ap.add_argument("--component", default="adaptive_rerank_instruction")
    ap.add_argument("--cell-name", required=True)
    ap.add_argument("--cell-dir", type=Path, required=True)
    ap.add_argument("--output-dir", type=Path, required=True)
    ap.add_argument("--valid-rows", type=Path, default=Path("/mnt/songzijun/Capability_Evolution/SCAPE-EasyOPD/outputs/component_sweep_0818/h100_2/components/adaptive_rerank_instruction/OPD_VALID_ROWS.jsonl"))
