    ...
    def _load_config(
        ...
        if config_path is not None:
            ...
            try:
                ...
                # Fallback: try omegaconf, then a tiny built-in YAML reader.
                try:
                    ...
                    from easyopd.config_loader import _load_minimal_yaml
...
class MethodInstance:
    ...
    def train(self, *, dry_run: bool = False, output_dir: str | None = None, extra_args: Optional[dict[str, Any]] = None) -> Any:
        """Launch the method's default training entrypoint when available."""
        trainer = getattr(self.method_cls, "train", None)
        if callable(trainer):
            return trainer(self.config, dry_run=dry_run, output_dir=output_dir, extra_args=extra_args or {})
        if self.method_name == "scape_component_opd":
            from pathlib import Path
            import subprocess
            import sys

            script = Path(__file__).resolve().parents[1] / "scripts" / "scape_component_opd.py"
            cmd = [sys.executable, str(script), "train"]
            if dry_run:
                cmd.append("--dry-run")
            if output_dir:
        ...
        raise NotImplementedError(f"Method '{self.method_name}' does not expose a train() entrypoint")
