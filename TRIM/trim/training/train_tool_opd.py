...
from trim.collection.same_state import coalition_label, resolve_component_ids
from trim.common.manifest import build_run_manifest, finalize_run_manifest, write_run_manifest
from trim.common.status import write_status_live
from trim.training.harness_dropout import DropoutSchedule
from trim.training.teacher import FullViewTeacher, TeacherConfig
from trim.training.tool_opd import learnability_score, tool_opd_loss
...
def run_micro_distill(
    *,
    ...
    When dry_run=False, uses trim.training.hf_tool_opd (true tool-token KL).
    ...
    manifest = build_run_manifest(
        ...
        stage="L",
        command=["python", "-m", "trim.training.train_tool_opd"],
        repo_root=repo,
        ...
        extra={
            ...
            "loss_impl": (
                "trim.training.hf_tool_opd:sr_opd_ce"
                if opd_mode == "sr_opd"
                else "trim.training.hf_tool_opd:tool_token_kl"
            ),
    ...
    if not dry_run:
        from trim.collection.same_state import collect_same_state_dataset, load_same_state_jsonl
        from trim.training.hf_tool_opd import ScapeHFToolOPD, run_tool_opd_train
