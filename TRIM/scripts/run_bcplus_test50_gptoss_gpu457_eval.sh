...
main() {
  ...
  log "RUN_ID=${RUN_ID} model=${MODEL_SLUG} zero_gpu=${GPU} tp=${TP} all_actors=${ALL_ACTOR_GPUS} verify=${ALL_VERIFY_GPU} all_eval_tp=${ALL_EVAL_TP} skip_zero=${SKIP_ZERO:-0}"

  if [[ "${SKIP_ZERO:-0}" != "1" ]]; then
    start_zero_stack
    run_eval zero "${ZERO_ACTOR_URL}" "${TP}" "gpu${GPU}"
  else
    log "SKIP_ZERO=1 — reusing existing zero results in ${PKG}/results/${RUN_ID}/zero"
  fi
