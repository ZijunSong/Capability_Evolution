...
from scape.eval.harmony_runtime import CANONICAL_STOP_TOKEN_IDS, O200K_HARMONY, decode_ids
from scape.training.hf_rl_opd_client import CISPO_MAX_ACTION_TOKENS

WORKER_MODULE = "scape.training.vllm_rollout_worker"
...
class VLLMGenerateClient:
    ...
    def start(self) -> None:
        ...
        scape_root = str(Path(__file__).resolve().parents[2])
        env["PYTHONPATH"] = scape_root + os.pathsep + env.get("PYTHONPATH", "")
...
class HFGenerateClient:
    ...
    def generate_batch(self, requests: Sequence[GenerateRequest]) -> list[GenerateResult]:
        from scape.training.four_cell_runtime import generate_harmony
