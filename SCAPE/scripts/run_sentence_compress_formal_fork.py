#!/usr/bin/env python3
"""Formal sentence_compress same-state K4/K8 reward fork.
    ...
    def snapshot(self) -> EnvironmentSnapshot:
        ...
        return capture_snapshot(
            ...
            working_memory={
                ...
                "sentence_compress": f"remaining={max(0, 8192 - self.cost * 256)}",
...
def collect_states_mode(args: argparse.Namespace) -> int:
    out = args.out_dir
    ...
    states_path = args.states_path or (out / "manifests" / f"{args.component}_states_n{args.n_states}_seed{args.seed}.jsonl")
    ...
    with states_path.open("w", encoding="utf-8") as f:
        for qid in qids:
            base = LiveState(qid=qid, query=queries[qid], gold=qrels.get(qid, set()), searcher=searcher, component=component, branch_seed=f"collect:{component}:{qid}")
            for _ in range(8):
                a_s, d_s, _ = policy_action(base, scorer, renderer, component=component, full=False)
                a_t, d_t, _ = policy_action(base, scorer, renderer, component=component, full=True)
                div = action_distance(a_s, a_t)
                if a_s.get("name") != a_t.get("name") or div >= ARG_THRESHOLD:
                    snap = base.snapshot()
                    item = {
                    ...
                    n += 1
                    if n % 8 == 0:
                        write_status_live(out / "STATUS_LIVE.md", stage="sentence_compress_collect_states", run_id="sentence_compress_formal_fork_0820", n_expected=args.n_states, n_finished=n, errors=[], extra={"component": component, "phase": "collect_states", "states_path": str(states_path)})
                    if n >= args.n_states:
                        print(json.dumps({"component": component, "n_states": n, "states_path": str(states_path)}, indent=2), flush=True)
                        return 0
                base.execute(a_s)
    print(json.dumps({"component": component, "n_states": n, "states_path": str(states_path), "status": "insufficient_candidate_states"}, indent=2), flush=True)
    return 0 if n >= args.n_states else 2


def load_frozen_states(path: Path) -> list[dict[str, Any]]:
    with path.open(encoding="utf-8") as f:
...
def run_utility_shard(args: argparse.Namespace) -> int:
    ...
    states = load_frozen_states(args.states_path) if args.states_path else collect_candidate_states(component, qids, queries, qrels, searcher, scorer, renderer, args.n_states)
    rows = []
    with shard_path.open("w", encoding="utf-8") as f:
        for idx, item in enumerate(states):
            ...
            if idx % 8 == 0:
                write_status_live(status_path, stage="sentence_compress_formal_fork", run_id="sentence_compress_formal_fork", n_expected=args.n_states, n_finished=idx + 1, errors=[], extra={"component": component, "K": k, "phase": "utility_shard", "gpu_device": args.device})
...
def aggregate(args: argparse.Namespace) -> int:
    ...
    with (out / "SENTENCE_COMPRESS_VALUE_PER_STATE.jsonl").open("w", encoding="utf-8") as f:
        for r in util_rows:
    ...
    with (out / "SENTENCE_COMPRESS_UTILITY_SUMMARY.csv").open("w", encoding="utf-8", newline="") as f:
    ...
    decision_payload = {"decision": decision, "component": "sentence_compress", "contract": "same xi_t; sentence_compress ON full branch vs OFF reduced branch; both continuations reduced policy; no full-harness takeover", "split": "UTILITY_LIVE256", "seed": args.seed, "runner": "true_live_fork_replay_hf_bm25", "full_harness_takeover": False, "rows": summary, "ranking": ranking}
    (out / "SENTENCE_COMPRESS_K4_K8_GATE.json").write_text(json.dumps(decision_payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    lines = ["# SENTENCE_COMPRESS_K4_K8_GATE", "", f"- decision: `{decision}`", "- runner: `true_live_fork_replay_hf_bm25`", "- full_harness_takeover: false", "- replay_noise: measured N1/N2 same-action branches, not assumed zero", "", "| component | mean live utility | replay noise | effect/noise | K4/K8 consistent |", "|---|---:|---:|---:|---|"]
    for r in ranking:
    ...
    (out / "SENTENCE_COMPRESS_K4_K8_GATE.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    for comp, fname in [("sentence_compress", "SENTENCE_COMPRESS_LIVE_UTILITY.md")]:
        rows = by_comp[comp]
    ...
    (pre / "SENTENCE_COMPRESS_FORMAL_FORK_HANDOFF.json").write_text(json.dumps(decision_payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    manifest = build_run_manifest(run_id="sentence_compress_formal_fork_0820", stage="sentence_compress_formal_fork", command=sys.argv, repo_root=REPO, output_dir=out, input_paths={"utility_live_manifest": out / "manifests" / "UTILITY_LIVE256.json"}, extra={"runner": "true_live_fork_replay_hf_bm25", "components": COMPONENTS, "K": [4,8], "n_states_per_component": args.n_states, "full_harness_takeover": False})
    ...
    write_status_live(out / "STATUS_LIVE.md", stage="sentence_compress_formal_fork", run_id="sentence_compress_formal_fork_0820", n_expected=8, n_finished=8, errors=[], extra={"phase": "quality-complete", "decision": decision, "runner": "true_live_fork_replay_hf_bm25"})
...
def main() -> int:
    ...
    ap.add_argument("--mode", choices=["collect-states", "utility", "noise", "aggregate"], required=True)
    ap.add_argument("--component", choices=COMPONENTS, default="sentence_compress")
    ...
    ap.add_argument("--out-dir", type=Path, default=REPO / "outputs" / "0820_sentence_compress_formal_fork")
    ap.add_argument("--states-path", type=Path)
    ...
    if args.mode == "collect-states":
        if not args.component:
            raise SystemExit("--component required for --mode collect-states")
        return collect_states_mode(args)
