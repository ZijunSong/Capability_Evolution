#!/usr/bin/env python3
"""Formal token_budget_marker same-state K4/K8 reward fork.
...
COMPONENTS = ("token_budget_marker",)
ARG_THRESHOLD = 0.12
...
class HFContinuationScorer:
    def __init__(self, model_path: str, *, device: str = "cuda:0", dtype: str = "bfloat16", max_prompt_tokens: int = 3072, attn_implementation: str = "flex_attention") -> None:
        import torch
    ...
    def sequence_logprob(self, prompt: str, continuation: str) -> float:
        ...
        with torch.inference_mode():
            logits = self.model(input_ids=ids, logits_to_keep=len(cids) + 1).logits
            logp = torch.nn.functional.log_softmax(logits[0, :-1, :], dim=-1)
        vals = []
        for j, tok in enumerate(cids):
            vals.append(float(logp[j, tok].detach().cpu()))
...
def pressure_render(snapshot: EnvironmentSnapshot, mask: Mapping[str, bool]) -> dict[str, Any]:
    """Render the complete real-corpus candidate context for budget-pressure forks."""
    payload = default_render(snapshot, mask)
    payload["documents"] = deepcopy(snapshot.working_memory.get("documents") or [])
    payload["curated_ids"] = list(snapshot.working_memory.get("curated_ids") or [])
    if mask.get("token_budget_marker"):
        payload["token_budget_marker"] = snapshot.working_memory.get("token_budget_marker")
        payload["pressure_metadata"] = deepcopy(snapshot.working_memory.get("pressure_metadata") or {})
    return payload
...
class LiveState:
    def __init__(self, *, qid: str, query: str, gold: set[str], searcher: Any, component: str, branch_seed: str = "") -> None:
        ...
        self.pressure_metadata: dict[str, Any] = {}
        self.token_budget_marker = f"remaining={8192}"
    ...
    def clone(self, suffix: str) -> "LiveState":
        ...
        new.pressure_metadata = deepcopy(self.pressure_metadata); new.token_budget_marker = self.token_budget_marker
    ...
    def snapshot(self) -> EnvironmentSnapshot:
        ...
        return capture_snapshot(
            ...
            working_memory={
                ...
                "token_budget_marker": self.token_budget_marker if self.pressure_metadata else f"remaining={max(0, 8192 - self.cost * 256)}",
                ...
                "pressure_metadata": deepcopy(self.pressure_metadata),
            },
...
def state_from_snapshot(snap_dict: Mapping[str, Any], query: str, gold: set[str], searcher: Any, component: str) -> LiveState:
    ...
    st.pressure_metadata = deepcopy(wm.get("pressure_metadata") or {})
    st.token_budget_marker = str(wm.get("token_budget_marker") or "")
...
def run_utility_shard(args: argparse.Namespace) -> int:
    ...
    scorer = HFContinuationScorer(args.model, device=args.device, dtype=args.dtype, max_prompt_tokens=args.max_prompt_tokens, attn_implementation=args.attn_implementation)
    renderer = DualViewRenderer(render_fn=pressure_render if args.pressure_mode else None)
    ...
    with shard_path.open("w", encoding="utf-8") as f:
        for idx, item in enumerate(states):
            ...
            if "a_S" not in item or "a_T" not in item:
                a_s, d_s, _ = policy_action(start, scorer, renderer, component=component, full=False)
                a_t, d_t, _ = policy_action(start, scorer, renderer, component=component, full=True)
                item["a_S"] = a_s; item["a_T"] = a_t
                item["P_tool_reduced"] = d_s["tool_name_probs"]; item["P_tool_full"] = d_t["tool_name_probs"]
                item["divergence"] = action_distance(a_s, a_t)
                item["divergence_type"] = "tool-name" if a_s.get("name") != a_t.get("name") else "args-only"
            ...
            s_final, s_trace = run_branch(start, item["a_S"], k=k, scorer=scorer, renderer=renderer, component=component, label="S", continuation_full=False)
            t_final, t_trace = run_branch(start, item["a_T"], k=k, scorer=scorer, renderer=renderer, component=component, label="T", continuation_full=args.teacher_continuation_full)
            ...
            row = {
                ...
                "continuation_policy": "full_teacher_reduced_student" if args.teacher_continuation_full else "reduced",
                "teacher_continuation_policy": "full_always" if args.teacher_continuation_full else "reduced_always",
                "student_continuation_policy": "reduced_always",
                "frozen_first_actions": True,
                "context_retention_policy": "successful_reads_append_only_retained_to_endpoint",
                "pressure_metadata": deepcopy(start.pressure_metadata),
                "token_budget_marker": start.token_budget_marker,
                "marker_visible_first_action": {"teacher": True, "student": False},
                "attention_implementation": args.attn_implementation,
                "max_prompt_tokens": args.max_prompt_tokens,
                "branch_S_endpoint": se,
                "branch_T_endpoint": te,
            ...
            if idx % 8 == 0:
                write_status_live(status_path, stage="token_budget_marker_formal_fork", run_id="token_budget_marker_formal_fork", n_expected=args.n_states, n_finished=idx + 1, errors=[], extra={"component": component, "K": k, "phase": "utility_shard", "gpu_device": args.device})
...
def aggregate(args: argparse.Namespace) -> int:
    ...
    with (out / "TOKEN_BUDGET_MARKER_VALUE_PER_STATE.jsonl").open("w", encoding="utf-8") as f:
        for r in util_rows:
    ...
    with (out / "TOKEN_BUDGET_MARKER_UTILITY_SUMMARY.csv").open("w", encoding="utf-8", newline="") as f:
    ...
    decision_payload = {"decision": decision, "component": "token_budget_marker", "contract": "same xi_t; token_budget_marker ON full branch vs OFF reduced branch; both continuations reduced policy; no full-harness takeover", "split": "UTILITY_LIVE256", "seed": args.seed, "runner": "true_live_fork_replay_hf_bm25", "full_harness_takeover": False, "rows": summary, "ranking": ranking}
    (out / "TOKEN_BUDGET_MARKER_K4_K8_GATE.json").write_text(json.dumps(decision_payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    lines = ["# TOKEN_BUDGET_MARKER_K4_K8_GATE", "", f"- decision: `{decision}`", "- runner: `true_live_fork_replay_hf_bm25`", "- full_harness_takeover: false", "- replay_noise: measured N1/N2 same-action branches, not assumed zero", "", "| component | mean live utility | replay noise | effect/noise | K4/K8 consistent |", "|---|---:|---:|---:|---|"]
    for r in ranking:
    ...
    (out / "TOKEN_BUDGET_MARKER_K4_K8_GATE.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    for comp, fname in [("token_budget_marker", "TOKEN_BUDGET_MARKER_LIVE_UTILITY.md")]:
        rows = by_comp[comp]
    ...
    (pre / "TOKEN_BUDGET_MARKER_FORMAL_FORK_HANDOFF.json").write_text(json.dumps(decision_payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    manifest = build_run_manifest(run_id="token_budget_marker_formal_fork_0820", stage="token_budget_marker_formal_fork", command=sys.argv, repo_root=REPO, output_dir=out, input_paths={"utility_live_manifest": out / "manifests" / "UTILITY_LIVE256.json"}, extra={"runner": "true_live_fork_replay_hf_bm25", "components": COMPONENTS, "K": [4,8], "n_states_per_component": args.n_states, "full_harness_takeover": False})
    ...
    write_status_live(out / "STATUS_LIVE.md", stage="token_budget_marker_formal_fork", run_id="token_budget_marker_formal_fork_0820", n_expected=8, n_finished=8, errors=[], extra={"phase": "quality-complete", "decision": decision, "runner": "true_live_fork_replay_hf_bm25"})
...
def main() -> int:
    ...
    ap.add_argument("--component", choices=COMPONENTS, default="token_budget_marker")
    ...
    ap.add_argument("--out-dir", type=Path, default=REPO / "outputs" / "0820_token_budget_marker_formal_fork")
    ...
    ap.add_argument("--attn-implementation", default="flex_attention", choices=["flex_attention", "eager", "flash_attention_2"])
    ap.add_argument("--max-prompt-tokens", type=int, default=30720)
    ap.add_argument("--states-cache", type=Path)
    ap.add_argument("--state-offset", type=int, default=0)
    ap.add_argument("--pressure-mode", action="store_true")
    ap.add_argument("--teacher-continuation-full", action="store_true", help="Use full component-on view for every Teacher continuation step; Student remains reduced.")
    args = ap.parse_args()
