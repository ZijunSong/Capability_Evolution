...
def main():
    ...
    manifest_src='outputs/h100_2_real_closed_loop_bm25_0816/REAL_CLOSED_LOOP_PER_QUERY.jsonl'
    specs=[
    ...
    for gpu,label,adapter in specs:
        ...
        cmd=[args.python,str(ROOT/'scripts/run_btp_auto_lora_real_closed_loop.py'),
          '--base-model',base,'--out-dir',str(out),'--query-manifest',manifest_src,
          '--n-queries',str(args.n_queries),'--split',args.split,'--seed',str(args.seed),
          '--max-steps',str(args.max_steps),'--device-map','cuda:0','--dtype','bfloat16',
          '--adapter',f'{label}={adapter}']
    ...
    (args.out/'FULL_EVAL_LAUNCH_MANIFEST.json').write_text(json.dumps({'status':'running','n_queries':args.n_queries,'seed':args.seed,'split':args.split,'jobs':entries},indent=2)+'\n')
    while procs:
        alive=[]
    ...
    (args.out/'FULL_EVAL_LAUNCH_MANIFEST.json').write_text(json.dumps({'status':'completed','n_queries':args.n_queries,'seed':args.seed,'split':args.split,'jobs':entries},indent=2)+'\n')
