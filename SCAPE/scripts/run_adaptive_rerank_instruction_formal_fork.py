#!/usr/bin/env python3
"""Formal adaptive_rerank_instruction same-state K4/K8 reward fork.
...
COMPONENTS = ("adaptive_rerank_instruction",)
ARG_THRESHOLD = 0.12
...
class HFContinuationScorer:
    ...
    def distribution(self, view: Mapping[str, Any], state: "LiveState") -> dict[str, Any]:
        ...
        for name in TOOL_NAMES:
            pids = self.tokenizer.encode(prompt, add_special_tokens=False)
            cids = self.tokenizer.encode(_call_text(name, state, view), add_special_tokens=False)
            if len(pids) > self.max_prompt_tokens:
                pids = pids[-self.max_prompt_tokens:]
            encoded.append((name, pids, cids))
        valid = [(name, pids, cids) for name, pids, cids in encoded if cids]
        if not valid:
            return {"tool_name_probs": {name: 1.0 / len(TOOL_NAMES) for name in TOOL_NAMES}, "decoded": action_for_tool(TOOL_NAMES[0], state, view), "sequence_logprobs": {name: 0.0 for name in TOOL_NAMES}}
        pad_id = self.tokenizer.pad_token_id
        if pad_id is None:
            pad_id = self.tokenizer.eos_token_id
        max_len = max(len(pids) + len(cids) for _, pids, cids in valid)
        rows = []
        starts = []
        for _, pids, cids in valid:
            ids = pids + cids
            starts.append((len(pids) - 1, len(cids)))
            rows.append(ids + [pad_id] * (max_len - len(ids)))
        ids_tensor = torch.tensor(rows, dtype=torch.long, device=self.device)
        with torch.inference_mode():
            logits = self.model(input_ids=ids_tensor).logits
            logp = torch.nn.functional.log_softmax(logits[:, :-1, :], dim=-1)
            values = []
            for row_idx, ((_, _, cids), (start, n_tokens)) in enumerate(zip(valid, starts)):
                token_ids = torch.tensor(cids, dtype=torch.long, device=self.device)
                values.append(float(logp[row_idx, start:start + n_tokens].gather(1, token_ids[:, None]).sum().detach().cpu()))
        scores = {name: value for (name, _, _), value in zip(valid, values)}
        scores.update({name: -float("inf") for name in TOOL_NAMES if name not in scores})
        m = max(scores.values())
    ...
    for qid in qids:
        ...
        for t in range(8):
            ...
            states.append({
                ...
                "divergence": div,
                "divergence_type": "tool-name" if a_s.get("name") != a_t.get("name") else "args-only" if div >= ARG_THRESHOLD else "same-action",
            })
...
def run_utility_shard(args: argparse.Namespace) -> int:
    ...
    with shard_path.open("w", encoding="utf-8") as f:
        for idx, item in enumerate(states):
            ...
            if idx % 8 == 0:
                write_status_live(status_path, stage="adaptive_rerank_instruction_formal_fork", run_id="adaptive_rerank_instruction_formal_fork", n_expected=args.n_states, n_finished=idx + 1, errors=[], extra={"component": component, "K": k, "phase": "utility_shard", "gpu_device": args.device})
...
def aggregate(args: argparse.Namespace) -> int:
    ...
    with (out / "ADAPTIVE_RERANK_INSTRUCTION_VALUE_PER_STATE.jsonl").open("w", encoding="utf-8") as f:
        for r in util_rows:
    ...
    with (out / "ADAPTIVE_RERANK_INSTRUCTION_UTILITY_SUMMARY.csv").open("w", encoding="utf-8", newline="") as f:
    ...
    decision_payload = {"decision": decision, "component": "adaptive_rerank_instruction", "contract": "same xi_t; adaptive_rerank_instruction ON full branch vs OFF reduced branch; both continuations reduced policy; no full-harness takeover", "split": "UTILITY_LIVE256", "seed": args.seed, "runner": "true_live_fork_replay_hf_bm25", "full_harness_takeover": False, "rows": summary, "ranking": ranking}
    (out / "ADAPTIVE_RERANK_INSTRUCTION_K4_K8_GATE.json").write_text(json.dumps(decision_payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    lines = ["# ADAPTIVE_RERANK_INSTRUCTION_K4_K8_GATE", "", f"- decision: `{decision}`", "- runner: `true_live_fork_replay_hf_bm25`", "- full_harness_takeover: false", "- replay_noise: measured N1/N2 same-action branches, not assumed zero", "", "| component | mean live utility | replay noise | effect/noise | K4/K8 consistent |", "|---|---:|---:|---:|---|"]
    for r in ranking:
    ...
    (out / "ADAPTIVE_RERANK_INSTRUCTION_K4_K8_GATE.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    for comp, fname in [("adaptive_rerank_instruction", "ADAPTIVE_RERANK_INSTRUCTION_LIVE_UTILITY.md")]:
        rows = by_comp[comp]
    ...
    (pre / "ADAPTIVE_RERANK_INSTRUCTION_FORMAL_FORK_HANDOFF.json").write_text(json.dumps(decision_payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    manifest = build_run_manifest(run_id="adaptive_rerank_instruction_formal_fork_0820", stage="adaptive_rerank_instruction_formal_fork", command=sys.argv, repo_root=REPO, output_dir=out, input_paths={"utility_live_manifest": out / "manifests" / "UTILITY_LIVE256.json"}, extra={"runner": "true_live_fork_replay_hf_bm25", "components": COMPONENTS, "K": [4,8], "n_states_per_component": args.n_states, "full_harness_takeover": False})
    ...
    write_status_live(out / "STATUS_LIVE.md", stage="adaptive_rerank_instruction_formal_fork", run_id="adaptive_rerank_instruction_formal_fork_0820", n_expected=8, n_finished=8, errors=[], extra={"phase": "quality-complete", "decision": decision, "runner": "true_live_fork_replay_hf_bm25"})
...
def main() -> int:
    ...
    ap.add_argument("--component", choices=COMPONENTS, default="adaptive_rerank_instruction")
    ...
    ap.add_argument("--out-dir", type=Path, default=REPO / "outputs" / "0820_adaptive_rerank_instruction_formal_fork")
