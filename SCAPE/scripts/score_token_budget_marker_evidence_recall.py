#!/usr/bin/env python3
"""Independently score token_budget_marker endpoint evidence recall forks."""
...
def main() -> int:
    ...
    for row in raw:
        ...
        row["state_id"] = f"token_budget_marker_K{row['K']}_{cache_index[row['snapshot_hash']]:03d}"
    ...
    with (args.out_dir / "TOKEN_BUDGET_MARKER_EVIDENCE_RECALL_PER_STATE.jsonl").open("w", encoding="utf-8") as handle:
        for row in sorted(scored, key=lambda item: (item["K"], item["state_id"])):
            handle.write(json.dumps(row, ensure_ascii=False) + "\n")
    (args.out_dir / "TOKEN_BUDGET_MARKER_EVIDENCE_RECALL_SUMMARY.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
