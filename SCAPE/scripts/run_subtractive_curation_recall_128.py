#!/usr/bin/env python3
"""Formal subtractive_curation same-state K4/K8 reward fork.
...
COMPONENTS = ("subtractive_curation",)
ARG_THRESHOLD = 0.12
    ...
    def snapshot(self) -> EnvironmentSnapshot:
        ...
        return capture_snapshot(
            ...
            working_memory={
                ...
                "subtractive_curation": f"remaining={max(0, 8192 - self.cost * 256)}",
...
def run_utility_shard(args: argparse.Namespace) -> int:
    ...
    if args.states_cache and args.states_cache.exists():
        ...
        cache_changed = False
        for item in states:
            if item.get("action_component") == component:
                continue
            start = state_from_snapshot(
            ...
            item.update({
                ...
                "P_tool_reduced": d_s["tool_name_probs"],
                "P_tool_full": d_t["tool_name_probs"],
                "divergence": action_distance(a_s, a_t),
                "divergence_type": "tool-name" if a_s.get("name") != a_t.get("name") else "args-only",
            })
            cache_changed = True
        if cache_changed:
        ...
        for state in states:
            state["action_component"] = component
    ...
    with shard_path.open("w", encoding="utf-8") as f:
        for idx, item in enumerate(states):
            ...
            # The frozen snapshot hash is the canonical initial-state identity.
            # Re-serializing after restore changes non-state branch metadata.
            ...
            if idx % 8 == 0:
                write_status_live(status_path, stage="subtractive_curation_recall_128", run_id="subtractive_curation_recall_128", n_expected=args.n_states, n_finished=idx + 1, errors=[], extra={"component": component, "K": k, "phase": "utility_shard", "gpu_device": args.device})
...
def aggregate(args: argparse.Namespace) -> int:
    ...
    with (out / "SUBTRACTIVE_CURATION_RECALL_VALUE_PER_STATE.jsonl").open("w", encoding="utf-8") as f:
        for r in util_rows:
    ...
    with (out / "SUBTRACTIVE_CURATION_RECALL_UTILITY_SUMMARY.csv").open("w", encoding="utf-8", newline="") as f:
    ...
    decision_payload = {"decision": decision, "component": "subtractive_curation", "contract": "same xi_t; subtractive_curation ON full branch vs OFF reduced branch; both continuations reduced policy; no full-harness takeover", "split": "UTILITY_LIVE256", "seed": args.seed, "runner": "true_live_fork_replay_hf_bm25", "full_harness_takeover": False, "rows": summary, "ranking": ranking}
    (out / "SUBTRACTIVE_CURATION_RECALL_K4_K8_GATE.json").write_text(json.dumps(decision_payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    lines = ["# SUBTRACTIVE_CURATION_RECALL_K4_K8_GATE", "", f"- decision: `{decision}`", "- runner: `true_live_fork_replay_hf_bm25`", "- full_harness_takeover: false", "- replay_noise: measured N1/N2 same-action branches, not assumed zero", "", "| component | mean live utility | replay noise | effect/noise | K4/K8 consistent |", "|---|---:|---:|---:|---|"]
    for r in ranking:
    ...
    (out / "SUBTRACTIVE_CURATION_RECALL_K4_K8_GATE.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    for comp, fname in [("subtractive_curation", "SUBTRACTIVE_CURATION_RECALL_LIVE_UTILITY.md")]:
        rows = by_comp[comp]
    ...
    (pre / "SUBTRACTIVE_CURATION_RECALL_FORMAL_FORK_HANDOFF.json").write_text(json.dumps(decision_payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    manifest = build_run_manifest(run_id="subtractive_curation_recall_128_0820", stage="subtractive_curation_recall_128", command=sys.argv, repo_root=REPO, output_dir=out, input_paths={"utility_live_manifest": out / "manifests" / "UTILITY_LIVE256.json"}, extra={"runner": "true_live_fork_replay_hf_bm25", "components": COMPONENTS, "K": [4,8], "n_states_per_component": args.n_states, "full_harness_takeover": False})
    ...
    write_status_live(out / "STATUS_LIVE.md", stage="subtractive_curation_recall_128", run_id="subtractive_curation_recall_128_0820", n_expected=8, n_finished=8, errors=[], extra={"phase": "quality-complete", "decision": decision, "runner": "true_live_fork_replay_hf_bm25"})
...
def main() -> int:
    ...
    ap.add_argument("--component", choices=COMPONENTS, default="subtractive_curation")
    ...
    ap.add_argument("--out-dir", type=Path, default=REPO / "outputs" / "0820_subtractive_curation_recall_128")
