# H100-2：Retrieval / Runtime Components Sweep — Qwen3 Fast-Start OPD + 5K Formal Protocol
...
# 11. H100-2 专属：Retrieval / Runtime Components
...
1. content_dedup
2. chunk_neighbors
3. adaptive_rerank_instruction
```

本机首要任务是区分“runtime 自动改变环境”与“Student 可通过 action 复现”的边界。
...
```text
AUTOMATIC_POOL_FILTER / PARTIAL
```

此前出现过 zero-trigger support，本轮不能靠 synthetic duplicate 修复。

### TRAIN-side duplicate index

在正式 rollout 前，只对 TRAIN corpus 离线建立：

```text
exact duplicate clusters
MinHash/shingle near-duplicate clusters
threshold = Harness-1 runtime 的真实 threshold
```

输出：

```text
manifests/component_sweep_5k/content_dedup/REAL_DUPLICATE_CLUSTER_INDEX.jsonl
```

这个 index 只用于 query prioritization，不直接产生 event rows。

5K event 必须是 Student 真实 search/read 轨迹进入 duplicate condition 后，Harness-1 dedup hook 真实触发。

若 Student-native 行为可投影，只允许：
...
禁止监督 Student 去删除一个它 action space 中不能删除的 environment object。
...
## 11.2 `chunk_neighbors`

先在 Harness-1 代码审计：

```text
neighbor chunks 如何追加
parent_doc/chunk id 是否 Student 可见
Student 能否通过 read/search 得到同样内容
```

### Case A：Student-native 可访问

例如可通过：

```text
read_document(parent_doc)
search_corpus(refined_query)
```
...
```text
PROJECTABLE/PARTIAL
L_OPD = projected action CE + next-turn reverse KL
```

5K row 的 `projectable_target` 必须是实际 parser 可执行 action。

### Case B：只能 runtime 注入

若 neighbor 信息无法通过任何 Student-native action 访问：

```text
NON_REALIZABLE_EXTERNAL_INFORMATION
```

仍收集 event support、Teacher/Before，但 Student After 两列 `N/A`。

## 11.3 `adaptive_rerank_instruction`

先拆成：

```text
instruction effect
retrieval ranking effect
```

对同一 query/state 记录：

```text
retrieved doc ids OFF
retrieved doc ids ON
topK overlap
qrel recall@K delta（仅 train-side diagnostic 或冻结 eval 中使用）
query refinement change
```

若只是 Teacher context/instruction 改变、不改 observation set：
...
若真实 retrieved docs/order 改变：

先判断 Student 是否可通过 better search query / curate action 合法复现。

可复现：

```text
PROJECTABLE
projected action CE + next-turn reverse KL
```

不可复现：

```text
PARTIAL / KEEP_RUNTIME
```

不得强做 same-state token KL。
...
```text
Wave 1:
GPU0-3  content_dedup 4 cells（仅 support gate pass 后）
GPU4-7  chunk_neighbors 4 cells（仅 realizability + support gate pass 后）

Wave 2:
GPU0-3  adaptive_rerank_instruction 4 cells
GPU4-7  evaluator/bootstrap/mechanism audit
```

被 gate 阻断释放出的 GPU 优先用于真实 collection / runtime audit。
...
```text
outputs/component_sweep_0818/h100_2/
├── content_dedup/
├── chunk_neighbors/
├── adaptive_rerank_instruction/
├── H1002_COMPONENT_ROWS.csv
├── H1002_COMPONENT_ROWS.json
├── H1002_COMPONENT_HANDOFF.json
└── SHA256SUMS
```
