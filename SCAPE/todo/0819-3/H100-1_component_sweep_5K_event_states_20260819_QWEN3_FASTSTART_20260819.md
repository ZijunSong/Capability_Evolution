# H100-1：Action / Projectable Components Sweep — Qwen3 Fast-Start OPD + 5K Formal Protocol
...
# 11. H100-1 专属：Action / Projectable Components
...
本机重点是验证 Harness side effect 能否转换成 Student-native action，因此 `projectable_target` 的正确性比“收满 5K”更优先。

## 11.1 `auto_populate_first_search`

分类：

```text
AUTOMATIC_SIDE_EFFECT / PROJECTABLE
```

真实 Harness effect：

```text
first successful search -> automatically add top-K docs to curated set
```
...
```text
Student rollout 中发生 first successful search，且 Harness-1 AUTO hook 对同一 pre-event state 产生非空、合法 delta_add
```
...
curated_ids_pre
curated_ids_teacher_post
delta_add
all(delta_add in Student-visible doc ids)
```

若 `delta_add` 含 Student 当前不可见 id：该 row 不可作为 projected supervision，记录 invalid_projection。
...
## 11.2 `importance_tagging`

分类：

```text
ARGUMENT_PRIVILEGE / PARTIAL-PROJECTABLE
```

禁止监督 `importance` 数值本身。Teacher privilege 必须投影为真实可执行的：

```text
importance-informed curation decision
-> add/remove choice
...
```text
Harness importance signal 实际改变或确定了一个可合法投影的 curate decision
```

每个 row 验证：

```text
added ids 对 Student 可见
removed ids 已在 Student 当前 curated set
projected action parser 可执行
```

若真实 valid add/remove event 到 Stage E 仍不足 5K：

```text
INSUFFICIENT_5K_EVENT_SUPPORT
```

若 Teacher utility 不正：

```text
TEACHER_COMPONENT_NO_POSITIVE_UTILITY
```

## 11.3 `subtractive_curation`
...
TRAIN_POOL construction 时可以优先保留 evidence-rich / curation-pressure query，但不得伪造满集合。rollout 中可做 TRAIN-only prioritization：

```text
curated_count >= 24
curated_count == capacity + new evidence arrives
```

只是优先采样真实状态，绝不能直接写 event row。
...
只有在各 component 的 5K gate 已通过后才启动下面的正式 4-cell 训练；`OPD_PILOT` 按上面的提前训练例外执行。

建议：

```text
Wave 1:
GPU0-3  AUTO: PURE42, PURE43, HYBRID42, HYBRID43
GPU4-7  importance: PURE42, PURE43, HYBRID42, HYBRID43

Wave 2:
GPU0-3  subtractive: PURE42, PURE43, HYBRID42, HYBRID43
GPU4-7  evaluator/bootstrap/mechanism audit
```

Phase U 时 GPU 优先用于真实 rollout collection，不允许提前启动正式 trainer。
...
若被 gate 阻断，不得创建一个假的 `TRAIN_STATES_5K.jsonl`；应保留 raw stats 并在 handoff 写明确 reason。
