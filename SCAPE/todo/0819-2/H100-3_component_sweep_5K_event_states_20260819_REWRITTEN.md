# H100-3：Framework Owner / Privileged Context Sweep — Harness-1 Integration + 5K Protocol
...
## 11.1 H100-3 先完成 framework unblock

在任何 component 正式 collection 前，本机依次完成：

```text
1. source setup_scape_easyopd_smoke7_env.sh
2. SCAPE Harness-1 runtime inventory
3. Harness1Bridge
4. openai/gpt-oss-20b Harmony adapter regression
5. real closed-loop collector
6. 446 -> 2,000 TRAIN_POOL builder
7. query leakage audit
8. collector acceptance
9. exact reverse-KL numeric regression
10. update H1003_SCAPE_EASYOPD_HANDOFF.json
```
...
FORMAL_COLLECTOR_REAL_RUNTIME_READY
TRAIN_POOL_2000_READY
GPT_OSS_HARMONY_READY
```
...
```text
same openai/gpt-oss-20b weights
evidence_graph ON
all other V8D OFF
```
...
response mask
tool span mask
Harmony serialization mask alignment
```
...
仅在 Phase U 完成且两个 component 各自 5K gate 通过后：
...
├── GPT_OSS_HARMONY_ACCEPTANCE.json
