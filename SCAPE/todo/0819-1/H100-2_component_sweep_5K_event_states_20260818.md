# H100-2：Retrieval / Runtime Components Sweep
...
这三类主要影响 retrieval / observation / document pool，因此集中处理。
...
之前旧数据出现过 zero trigger support，因此本次必须重新做真实 event-conditioned collection。

为达到 5K unique event-active states，本组件允许先用 train corpus 的真实 duplicate index 做 query prioritization；但所有选中 query 仍必须执行 2–4 次独立 Student on-policy rollout，并通过统一 `state_uid` 去重。

先离线扫描当前真实 corpus：

```text
exact duplicate
MinHash/shingle duplicate
threshold = runtime真实阈值
```

建立：

```text
REAL_DUPLICATE_CLUSTER_INDEX.jsonl
```

然后只用能真实触发这些 clusters 的 query 做 on-policy collection。

禁止人为复制文档制造 duplicate。
...
只有在 `n_unique_event_active >= 5,000` 后才进入训练。若有 support，Student-native supervision 只允许：
...
不能监督 Student 去“删除一个它没有 action 能删除的 environment pool object”。

PURE_OPD：

```text
projected behavior CE + next_turn_reverse_KL
```
...
```text
neighbor chunks 是如何被追加
Student 是否能通过 read/search 合法得到同样内容
document/chunk ids 是否可见
```

### 情况 A：可通过 Student-native action 获取
...
### 情况 B：neighbor 内容只由 runtime 直接注入，Student 没有任何 action 可以访问
...
Teacher metric 仍然运行并进入 10-component table；Student After 两列填 N/A。
...
```text
topK overlap ON vs OFF
qrel recall@K delta
retrieved doc ids delta
```

### 若只改变 Teacher context/instruction，不改变 environment observation set
...
### 若会改变真实 retrieved documents/order

必须判断 Student 是否能通过：

```text
better search query
better curate action
```
...
```text
GPU0-3: content_dedup 4 cells（前提 support gate pass）
GPU4-7: chunk_neighbors 4 cells（前提 realizability pass）
```
...
GPU4-7: evaluator / bootstrap / mechanism cases
```

如果前两个被 gate 阻断，立即把空闲 GPU 用于该组件的真实 event collection / evaluator，不允许额外跑别的 research branch。
...
`H1002_COMPONENT_ROWS.csv` 恰好 3 行。

对于 N/A 必须在 `reason` 字段解释：
