...
try:
    import tinker
except ModuleNotFoundError:  # local HF/PEFT closed-loop does not use hosted training
...
if os.environ.get("SCAPE_FORCE_LOCAL_HARMONY", "0") == "1":
    ...
    Conversation = HarmonyEncoding = Any
    HarmonyEncodingName = SimpleNamespace(HARMONY_GPT_OSS=None)
    Message = Any
    Role = SimpleNamespace(USER="user", ASSISTANT="assistant", SYSTEM="system")
...
class SlidingWindowSearchEnv(Env):
    ...
    def __init__(
        ...
        try:
            ...
            from harness._local_harmony_fallback import _LocalHarmonyEncodingFallback
            self.enc = _LocalHarmonyEncodingFallback()
