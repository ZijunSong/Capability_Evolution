...
if os.environ.get("SCAPE_FORCE_LOCAL_HARMONY", "0") == "1":
    ...
    from harness._local_harmony_fallback import (
        _LocalAuthor,
        _LocalContent,
        _LocalConversation,
        _LocalHarmonyEncodingFallback,
        _LocalMessage,
        _LocalReasoningEffort,
        _LocalRole,
        _LocalToolDescription,
    )

    Author = _LocalAuthor
    Conversation = _LocalConversation
    DeveloperContent = _LocalContent
    HarmonyEncoding = Any
    Message = _LocalMessage
    ReasoningEffort = _LocalReasoningEffort
    SystemContent = _LocalContent
    ToolDescription = _LocalToolDescription
    HarmonyEncodingName = SimpleNamespace(HARMONY_GPT_OSS=None)
    Role = _LocalRole

    def load_harmony_encoding(*args, **kwargs):
        return _LocalHarmonyEncodingFallback()
elif True:
    try:
        ...
        Author = Conversation = DeveloperContent = HarmonyEncoding = Message = ReasoningEffort = SystemContent = ToolDescription = None
        HarmonyEncodingName = SimpleNamespace(HARMONY_GPT_OSS=None)
        Role = SimpleNamespace(USER="user", ASSISTANT="assistant", SYSTEM="system", DEVELOPER="developer", TOOL="tool")
